...
public class Select : MonoBehaviour {
    ...
    public  float moveTime, distance;
    private string Plane;
    private string curPlane;
	...
	void Update () {
        Plane = GameObject.Find("Main Camera").GetComponent<CursorActivity>().a;
        if  (Plane !=null&& curPlane !=Plane )
        {
            planetName = Plane;
        ...
        {
           // GameObject.Find("Main Camera").GetComponent<CursorActivity>().isSelecting = false;
            count--;
            SwitchPlanet();
            SetMove(planetName);
            isMoving = true;
    ...
    void SwitchPlanet()
    {
        ...
        switch (count)
        {
            ...
            case 9: planetName = "Halley"; break;
            //case 10: planetName = "YunShi";break;
            case 10: planetName = "Moon"; break;
        }
    }

   public  void SetMove(string planetName)
    {
        ...
        direction = (Vector3.zero + new Vector3(0, 0, 0) - transform.localPosition).normalized;
        distance = (Vector3.zero + new Vector3(0, 0, 0) - transform.localPosition).magnitude;   //计算相机到行星的距离，用于确定移动步长
